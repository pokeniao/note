...
# 45.抽象类和接口的区别
1. **一个类**只能==继承一个抽象类==，但是可以实现==多个接口==
2. 
	- **抽象类**像是是一个父类，它定义了一些共同的特征和行为，即==子类是父类的一种特殊类型==。
	- ==**接口**更像是一种能力或技能==，类可以实现一个或多个接口，就像一个人可以掌握多种技能一样。实现了接口的类必须==提供这些方法的具体实现==。这种关系更==像是一个人具备了某些技能==或能力。
3. 
	- **抽象类**：==可以包含成员变量或方法==，==可以是抽象的或具体==的，子类可以继承这些成员变量，或具体实现，也可以覆盖。
	 - **接口**：==不能包含成员变量和方法==，只能包含方法定义，实现接口的类必须提供方法的具体实现。（JDK 1.8允许给接口添加非抽象的方法实现，但必须使用default关键字修饰；）