...
先通过**连接器**与客户端建立连接、获取权限。
第一是：**语法分析和解析**,确保SQL语句的语法正确性
第二是：**查询优化**，MySQL 会对 SQL 语句进⾏优化，==以确定最优的执⾏计划。==例如索引、表连接等
第三是：**查询执⾏**，==在查询优化后==，MySQL 开始执⾏==查询，读取和处理数据==。
最后: **结果返回**,MySQL 会将查询结果返回给客户端.
